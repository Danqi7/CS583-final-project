...
def train(
    ...
    if hold_one_out and hold_out in groups:
        local_losses = {f'{t0}:{t1}':[] for (t0, t1) in steps if hold_out not in [t0, t1]}
    else:
    ...
    if local_losses is None:
        if hold_one_out and hold_out in groups:
            ...
            if reverse_schema:
                local_losses = {
                    **local_losses,
                    **{f'{t0}:{t1}':[] for (t0, t1) in generate_steps(groups[::-1]) if hold_out not in [t0, t1]}
                }
        else:
